...
const dancers = Array.from({ length: 6 }, (_, i) => ({ group: $<SVGGElement>(`#dancer-${i}`), body: $<SVGGElement>(`#dancer-${i} .body`), left: $<SVGPathElement>(`#dancer-${i} .left`), right: $<SVGPathElement>(`#dancer-${i} .right`) }));
let progress = 0;
let cycle = 0;
...
function render() {
  ...
  $('#hands').innerHTML = state.hands.map(pair => `<polyline points="${pair.map(i => `${itemAt(state.dancers, i).x},${itemAt(state.dancers, i).y}`).join(' ')}"/>`).join('');

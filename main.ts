...
dancers = Array.from({ length: dance.frame(0, 0).dancers.length }, (_, i) => ({ group: $<SVGGElement>(`#dancer-${i}`), body: $<SVGGElement>(`#dancer-${i} .body`), left: $<SVGCircleElement>(`#dancer-${i} .left`), right: $<SVGCircleElement>(`#dancer-${i} .right`) }));
  showLetters();
}
let progress = 0;
let cycle = 0;
...
function render() {
  ...
  $('#hands').innerHTML = state.hands.map(({ dancers: [a, b], reach, arch = 0 }) => {
    ...
    return `<path d="${arm(from, to)} ${arm(to, from)}"/>`;
  }).join('');

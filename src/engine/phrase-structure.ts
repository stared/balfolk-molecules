...
export interface PhraseSpan { name: string; start: number; beats: number; depth: number }
...
export function phraseSpans(phrase: Phrase, start = 0, depth = 0): PhraseSpan[] {
  ...
  const spans: PhraseSpan[] = [{ name: phrase.name, start, beats: phraseBeats(phrase), depth }];
...
export interface PhraseRepeat { name: string; start: number; beats: number; times: number }
/** Preserve repetition as a grouping, separately from its expanded occurrences. */
export function phraseRepeats(phrase: Phrase, start = 0): PhraseRepeat[] {
  ...
  if (phrase.kind === 'repeat') {
    ...
    return [{ name: phrase.phrase.kind === 'repeat' ? 'Repeat' : phrase.phrase.name,
      start, beats: beats * phrase.times, times: phrase.times },
      ...Array.from({length: phrase.times}, (_, i) => phraseRepeats(phrase.phrase, start + i * beats)).flat()];
  }
  const repeats: PhraseRepeat[] = [];
  for (const part of phrase.parts) {
    repeats.push(...phraseRepeats(part, start));
    start += phraseBeats(part);
  }
  return repeats;
}

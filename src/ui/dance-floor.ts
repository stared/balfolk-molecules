...
interface DancerElement {
  ...
  body: SVGGElement;
  feet: SVGGElement;
  left: SVGEllipseElement;
  right: SVGEllipseElement;
  weight: number;
...
function renderThud(mark: SVGEllipseElement, age: number): void {
  const phase = Math.max(0, Math.min(1, age / thudDuration));
  mark.style.opacity = String(0.5 * (1 - phase) ** 2);
  // Fixed size, inside the silhouette: only the contact darkens, never a ripple.
}
function syncDancers(poses: Dancer[]): void {
  ...
  for(const d of poses)if(!dancers.has(d.id)){
    ...
    group.innerHTML=`<g class="body"><circle class="silhouette" r="14"/><g class="support"><ellipse class="left" cx="-5" cy="9" rx="3" ry="1.7"/><ellipse class="right" cx="5" cy="9" rx="3" ry="1.7"/></g><g class="eyes"><circle cx="-4" cy="-9" r="2.5"/><circle cx="4" cy="-9" r="2.5"/><circle class="pupil" cx="-4" cy="-9.8" r="1.2"/><circle class="pupil" cx="4" cy="-9.8" r="1.2"/></g></g><text text-anchor="middle" dy="4">${d.id}</text>`;
    $('#dancers').append(group);
    const body=group.querySelector<SVGGElement>('.body'),feet=group.querySelector<SVGGElement>('.support'),left=group.querySelector<SVGEllipseElement>('.left'),right=group.querySelector<SVGEllipseElement>('.right');
